  ...
  try {
    ...
    if (payload.category) {
      let category = await Category.findOne({
        nama: { $regex: payload.category, $options: "i" },
      });
      if (category) {
    ...
    if (payload.tags && payload.tags.length > 0) {
      let tags = await Tag.find({ nama: { $in: payload.tags } });
      if (tags.length) {
...
const index = async (req, res, next) => {
  try {
    ...
    if (tags.length) {
      let tagsResult = await Tag.find({ nama: { $in: tags } });
...
const destroy = async (req, res, next) => {
  try {
    ...
    let currentImage = `${config.rootPath}/
